...
<td>
If you try to run <code>StudentInfo.java</code>, you'll see an error message in the terminal indicating the program bombed due to the compilation problem.
</td>
</tr>
...
1. Remove the type `double` on line 23 to prevent a duplicate variable
   declaration. The compiler errors should go away.
2. Run `StudentInfo` to view the output.
3. Run `StudentInfoTest` to confirm your solution passes the two Junit tests:
   - One test checks the expected output. The other test counts the number of
     times `gpa` is declared, which should be just once.
...
Double click on `BusRoute.java` in the Package Explorer to open the file in the
...
Double click on `DebugIt.java` in the Package Explorer to open the file in the
editor, then run the program to view the output.
...
Double click on `Rounding.java` in the Package Explorer to open the file in the
editor.
